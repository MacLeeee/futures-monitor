...
const REGIME_COLORS: Record<GoldRegime, { text: string; border: string; bar: string }> = {
  "Cash Liquidation":          { text: "text-red-400",   border: "border-red-500/20",   bar: "bg-red-500" },
  "Rates-Dollar Bearish Gold": { text: "text-orange-400", border: "border-orange-500/20", bar: "bg-orange-500" },
  "Clean Bullish Gold":        { text: "text-emerald-400", border: "border-emerald-500/20", bar: "bg-emerald-500" },
  "Reflation Gold":            { text: "text-amber-400",  border: "border-amber-500/20",  bar: "bg-amber-500" },
  "Defensive Gold":            { text: "text-indigo-400", border: "border-indigo-500/20", bar: "bg-indigo-500" },
  "Fiscal / Debasement Hedge": { text: "text-purple-400", border: "border-purple-500/20", bar: "bg-purple-500" },
  ...
  "Mixed":                     { text: "text-[#9090a0]",  border: "border-[#9090a0]/20",  bar: "bg-[#585868]" },
};
...
function cardBg(score: number) {
  ...
  if (score >= 45) return "bg-amber-500/5";
...
function TrendChip({ label, sign }: { label: string; sign: TrendSign }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium font-mono ${color}`}>
      <span className="text-[10px] text-[#585868]">{label}</span>
      {arrow} {sign}
    </span>
  );
}
...
function Bar({ value, max, color, label }: { value: number; max: number; color: string; label: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] text-[#585868] w-16 text-right">{label}</span>
      <span className="text-xs font-mono text-[#e4e4ec] w-8 tabular-nums">{value}</span>
      <div className="flex-1 h-1.5 bg-[#1e1f2a] rounded-full overflow-hidden">
  ...
  return (
    <div className={`px-2 py-1.5 rounded-md text-[10px] font-medium flex items-center gap-1.5 transition-all ${
      active
        ...
        : "bg-[#0f1016] text-[#585868]"
    }`}>
      <span className={`w-1.5 h-1.5 rounded-full ${active ? "bg-amber-400" : "bg-[#585868]"}`} />
      <span>{label}</span>
...
function ETFRow({ ticker, price, chg15, chg60, chg240 }: {
  ...
  const fmt = (v: number | null) => {
    if (v === null) return <span className="text-[#585868] font-mono">—</span>;
    const c = v > 0 ? "text-emerald-400" : v < 0 ? "text-red-400" : "text-[#585868]";
    const sign = v > 0 ? "+" : "";
  ...
  return (
    <tr className="border-b border-[#1e1f2a] hover:bg-amber-500/5 transition-colors">
      <td className="py-2 px-3"><span className="text-xs font-mono font-medium text-[#9090a0]">{ticker}</span></td>
      <td className="py-2 px-3"><span className="text-[10px] text-[#585868]">{info.name}</span></td>
      <td className="py-2 px-3 text-right"><span className="text-xs font-mono tabular-nums text-[#e4e4ec]">{price.toFixed(2)}</span></td>
...
export default function GoldPage() {
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-[#08080c] text-[#e4e4ec]">
        <div className="max-w-screen-lg mx-auto p-4">
          <Link href="/" className="inline-flex items-center gap-1.5 text-xs text-[#585868] hover:text-[#9090a0] mb-6 transition-colors">
            <ArrowLeft size={12} /> 返回期货监控
          </Link>
          <div className="bg-[#0f1016] border border-[#1e1f2a] rounded-xl p-12 text-center">
            <AlertTriangle size={24} className="text-red-400 mx-auto mb-3" />
            <p className="text-[#9090a0] text-sm">数据暂不可用{error ? ` (${error})` : ""}</p>
            <button onClick={loadData} className="mt-4 px-4 py-1.5 text-xs bg-[#1e1f2a] rounded-md hover:bg-[#2a2b36] transition-colors text-[#9090a0]">
              重试
            </button>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#08080c] text-[#e4e4ec] font-sans">
      <div className="max-w-screen-lg mx-auto p-4 space-y-4">
        ...
        {/* 顶栏 */}
        <header className="flex items-center justify-between pb-3 border-b border-[#1e1f2a]">
          <div className="flex items-center gap-3">
            <Link href="/" className="text-[#585868] hover:text-[#9090a0] transition-colors">
              <ArrowLeft size={16} />
            </Link>
            <Zap size={16} className="text-amber-500" />
            <h1 className="text-sm font-bold tracking-tight text-amber-500">
              黄金宝宝巴士
            </h1>
            <span className="text-[10px] text-[#585868]">宏观监控 v2</span>
          </div>
          <div className="flex items-center gap-3">
            {data.timestamp && (
              <span className="text-[10px] text-[#585868] font-mono">
            ...
            <button onClick={loadData} className="p-1.5 rounded-md hover:bg-[#161720] text-[#585868] hover:text-[#9090a0] transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {/* 状态机 */}
          <div className={`bg-[#0f1016] border ${regimeStyle.border} rounded-xl p-4`}>
            <div className="flex items-center gap-2 mb-3">
              <Target size={13} className="text-[#585868]" />
              <span className="text-[10px] text-[#585868] font-medium uppercase tracking-widest">Gold Regime</span>
            </div>
            ...
            {data.regime_detail && (
              <div className="mb-2 flex items-center gap-3 text-[10px]">
                <span className="text-[#585868]">主逻辑</span>
                <span className="text-[#9090a0] font-medium">{data.regime_detail.dominant_theme}</span>
                <span className="text-emerald-400 font-mono">Bull {data.regime_detail.bull_max}</span>
                <span className="text-red-400 font-mono">Bear {data.regime_detail.bear_max}</span>
              </div>
            )}
            <p className="text-xs text-[#9090a0] leading-relaxed">{data.regime_guide}</p>
          </div>

          {/* 流动性 */}
          <div className={`${cardBg(data.liquidity_score)} border border-[#1e1f2a] rounded-xl p-4`}>
            <div className="flex items-center gap-2 mb-3">
              <Droplets size={13} className="text-[#585868]" />
              <span className="text-[10px] text-[#585868] font-medium uppercase tracking-widest">Liquidity Stress</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="relative w-20 h-20 shrink-0">
                <svg className="w-20 h-20 -rotate-90" viewBox="0 0 64 64">
                  <circle cx="32" cy="32" r="27" fill="none" className="stroke-[#1e1f2a]" strokeWidth="5"/>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span className="text-[8px] text-[#585868]">/100</span>
                </div>
              </div>
              <div className="flex-1">
                <span className={`text-sm font-semibold ${lc}`}>{data.liquidity_state}</span>
                <div className="mt-1 text-[10px] text-[#585868] leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {/* 趋势 */}
          <div className="bg-[#0f1016] border border-[#1e1f2a] rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <TrendingUp size={13} className="text-[#585868]" />
              <span className="text-[10px] text-[#585868] font-medium uppercase tracking-widest">GLD Trend</span>
            </div>
            <div className="flex items-center gap-2 mb-2">
              <TrendChip label="15m" sign={trend["15m"]} />
              <span className="text-[#1e1f2a] text-lg">→</span>
              <TrendChip label="1h" sign={trend["1h"]} />
              <span className="text-[#1e1f2a] text-lg">→</span>
              <TrendChip label="4h" sign={trend["4h"]} />
            </div>
            {data.combo_advice && (
              <p className="text-[10px] text-amber-400/80 mt-2">📋 {data.combo_advice}</p>
            )}
          </div>
          ...
          <div className="bg-[#0f1016] border border-[#1e1f2a] rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <Shield size={13} className="text-[#585868]" />
              <span className="text-[10px] text-[#585868] font-medium uppercase tracking-widest">Structure Score</span>
            </div>
        ...
        {/* ── ETF 全景 ──────────────────────────────── */}
        <div className="bg-[#0f1016] border border-[#1e1f2a] rounded-xl overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-[#1e1f2a]">
            <Activity size={13} className="text-[#585868]" />
            <span className="text-[10px] text-[#585868] font-medium uppercase tracking-widest">ETF Snapshot</span>
            <span className="ml-auto text-[9px] text-[#585868]">30 标的 · 15m/1h/4h 变化率</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-[#1e1f2a] text-[10px] text-[#585868] uppercase tracking-wider">
              ...
              <tbody>
                {etf ? TICKER_ORDER.map(t => (
                  ...
                  <tr><td colSpan={6} className="py-6 text-center text-[#585868] text-xs">ETF 快照暂不可用</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="bg-[#0f1016] border border-[#1e1f2a] rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <span className="text-[10px] text-[#585868] font-medium uppercase tracking-widest block mb-2">Trading Advice</span>
        ...
        {/* 页脚 */}
        <footer className="text-center text-[10px] text-[#585868] py-6 border-t border-[#1e1f2a]">
