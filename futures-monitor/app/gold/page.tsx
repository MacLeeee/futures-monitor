...
const REGIME_COLORS: Record<GoldRegime, { text: string; border: string; bar: string }> = {
  ...
  "Reflation Gold":            { text: "text-blue-500",  border: "border-blue-200",  bar: "bg-blue-600" },
  ...
  "Mixed":                     { text: "text-gray-500",  border: "border-[#9090a0]/20",  bar: "bg-gray-400" },
};
...
function cardBg(score: number) {
  ...
  if (score >= 45) return "bg-blue-50/40";
...
function Bar({ value, max, color, label }: { value: number; max: number; color: string; label: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs font-mono text-gray-900 w-8 tabular-nums">{value}</span>
...
function Flag({ active, label, desc }: { active: boolean; label: string; desc: string }) {
  return (
    <div className={`px-2 py-1.5 rounded-md text-[10px] font-medium flex items-center gap-1.5 transition-all ${
      active
        ? "bg-blue-50/70 text-blue-500 ring-1 ring-blue-300/30"
...
function ETFRow({ ticker, price, chg15, chg60, chg240 }: {
  ...
  const fmt = (v: number | null) => {
    ...
    const c = v > 0 ? "text-emerald-600" : v < 0 ? "text-red-600" : "text-gray-400";
    const sign = v > 0 ? "+" : "";
  ...
  return (
    <tr className="border-b border-gray-200 hover:bg-blue-50/40 transition-colors">
      <td className="py-2 px-3"><span className="text-xs font-mono font-medium text-gray-500">{ticker}</span></td>
      <td className="py-2 px-3"><span className="text-[10px] text-gray-400">{info.name}</span></td>
      <td className="py-2 px-3 text-right"><span className="text-xs font-mono tabular-nums text-gray-900">{price.toFixed(2)}</span></td>
    ...
    return (
      <div className="min-h-screen bg-[#f4f5f7] text-gray-900">
        <div className="max-w-screen-lg mx-auto p-4">
          <Link href="/" className="inline-flex items-center gap-1.5 text-xs text-gray-400 hover:text-gray-500 mb-6 transition-colors">
            <ArrowLeft size={12} /> 返回期货监控
          </Link>
          <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
            <AlertTriangle size={24} className="text-red-600 mx-auto mb-3" />
            <p className="text-gray-500 text-sm">数据暂不可用{error ? ` (${error})` : ""}</p>
            <button onClick={loadData} className="mt-4 px-4 py-1.5 text-xs bg-gray-200 rounded-md hover:bg-[#2a2b36] transition-colors text-gray-500">
              重试
            </button>
          </div>
        </div>
      ...
      <div className="max-w-screen-lg mx-auto p-4 space-y-4">
        ...
        <header className="flex items-center justify-between pb-3 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-sm font-bold tracking-tight text-blue-600">
              黄金宝宝巴士
            </h1>
            <span className="text-[10px] text-gray-400">宏观监控 v2</span>
          </div>
          <div className="flex items-center gap-3">
            {data.timestamp && (
            ...
            <button onClick={loadData} className="p-1.5 rounded-md hover:bg-gray-100 text-gray-400 hover:text-gray-500 transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className={`bg-white border ${regimeStyle.border} rounded-xl p-4`}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[10px] text-gray-400 font-medium uppercase tracking-widest">Gold Regime</span>
            </div>
            ...
            {data.regime_detail && (
              <div className="mb-2 flex items-center gap-3 text-[10px]">
                <span className="text-gray-400">主逻辑</span>
                <span className="text-gray-500 font-medium">{data.regime_detail.dominant_theme}</span>
            ...
            <p className="text-xs text-gray-500 leading-relaxed">{data.regime_guide}</p>
          </div>

          {/* 流动性 */}
          <div className={`${cardBg(data.liquidity_score)} border border-gray-200 rounded-xl p-4`}>
            <div className="flex items-center gap-2 mb-3">
              <Droplets size={13} className="text-gray-400" />
              <span className="text-[10px] text-gray-400 font-medium uppercase tracking-widest">Liquidity Stress</span>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[10px] text-gray-400 font-medium uppercase tracking-widest">GLD Trend</span>
            </div>
            ...
            {data.combo_advice && (
              <p className="text-[10px] text-blue-500/80 mt-2">📋 {data.combo_advice}</p>
            )}
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[10px] text-gray-400 font-medium uppercase tracking-widest">Structure Score</span>
            </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-200">
            ...
            <span className="text-[10px] text-gray-400 font-medium uppercase tracking-widest">ETF Snapshot</span>
            <span className="ml-auto text-[9px] text-gray-400">30 标的 · 15m/1h/4h 变化率</span>
          </div>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {etf ? TICKER_ORDER.map(t => (
                  ...
                  <tr><td colSpan={6} className="py-6 text-center text-gray-400 text-xs">ETF 快照暂不可用</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <span className="text-[10px] text-gray-400 font-medium uppercase tracking-widest block mb-2">Trading Advice</span>
