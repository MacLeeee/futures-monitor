...
export default function SignalPanel({ data }: SignalPanelProps) {
  ...
  return (
    <div className={`rounded-lg border ${
      hasSignal ? "border-amber-400/50 bg-amber-50/40" : "border-stone-200 bg-stone-50/60"
    }`}>
      <button
      ...
      >
        <div className="flex items-center gap-3">
          <Zap size={14} className={hasSignal ? "text-amber-500" : "text-stone-500"} />
          <span className="text-sm font-semibold text-stone-800">突破信号</span>
          <div className="flex items-center gap-2">
            ...
            {!hasSignal && !hasNear && (maFirstUp.length + maFirstDn.length === 0) && (
              <span className="text-xs text-stone-400">暂无满足条件的品种</span>
            )}
            {hasNear && (
              <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-stone-100 border border-stone-300 text-stone-400">
                <AlertTriangle size={9} /> 待观察 {nearLong.length + nearShort.length}
              </span>
            )}
            {(maFirstUp.length + maFirstDn.length) > 0 && (
              <span className="px-2 py-0.5 rounded-full text-xs bg-amber-100/40 border border-amber-200 text-amber-600">
  ...
  return (
    <div className={`flex items-center justify-between px-2.5 py-1.5 rounded text-xs font-mono ${
      ...
      <div className="flex items-center gap-1 text-[10px]">
        ...
        <span className={isLong ? "text-amber-500" : "text-amber-500"}>
